...
function findAdjacent(current, grid){
...
function findAllPaths(x,y, size, db, collection){
    ...
    function takeStep(currentCell){
        if(currentCell !== undefined){ //for testing
            function sTeP(){
                ...
                var testArray = findAdjacent(currentCell, grid);
            ...
            do{var result = sTeP(currentCell)}
            while(result === null);
            return result;
        } else {
    ...
    var grid, currentPath, solution_id;
    solution_id = 1;
    currentPath = [];
    grid = buildGrid(size);
    ...
    for(n=1;n<100;n++){  //start at 1 because one is already complete
        removeCells();
...
function main(){
    ...
    MongoClient.connect("mongodb://localhost:27017/slither_puzzle", function(err, db) {
      ...
      if(!err) {
        ...
        findAllPaths(1,1,5, db, collection);
      }
    });
    return
}
